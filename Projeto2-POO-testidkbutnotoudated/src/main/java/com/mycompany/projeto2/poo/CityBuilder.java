...
public class CityBuilder {
    ...
    public City createCity(Civilization civilization)throws CoordinatesNotSuitableException{
        ...
        if(checkCoordinates(civilization)){
     ...
     * @throws CoordinatesNotSuitableException informa que as coordenadas escolhidas não são adequadas
     */
    private boolean checkCoordinates(Civilization civ)throws CoordinatesNotSuitableException{
        ...
        else if(!isTerrainAdequate(coordX,coordY,civ)){
     ...
     */
    private boolean isTerrainAdequate(int coordX,int coordY, Civilization civ){
        int index = 1;
        while(index <= 3){
                    ...
                    if(map.getCellEntryCost(x+coordX, y+coordY) == -1 || map.getCellIsSomethingOnTop(x+coordX, y+coordY)){
                        if(civ == null) {
                            ...
                            for(int i=0;i<temp.size();i++) {
                                if(temp.get(i).getCoordX() != x+coordX || temp.get(i).getCoordY() != y+coordX) {
                                    return false;
                                }
                            }
