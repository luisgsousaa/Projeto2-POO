...
public class ProduceResources {
    ...
    public static void chooseProduceType(Cell currentCell,City city){
        if(currentCell.getFoodProduction() != 0){
            ...
            city.addGoldResources(produce(currentCell.getGoldProduction(), currentCell.getProductivity(), currentCell.getNumWorkers(), 1 + 0.5*Building.countType(city.getCiv().getBuildings(),0)));
